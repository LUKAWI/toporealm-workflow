...
import { assessTaskCompletion, recordVerification } from "../src/evidence.js";
import type { WorkflowExecutionReport, WorkflowTask } from "../src/domain.js";

// ---------- adjudication.test.ts 移植：写入建议指向 wf.verify-task（1.0 命令名） ----------
...
describe("workflow 裁决建议", () => {
  it("从真实任务双轴证据产生 independent 写入建议，不伪造 human", () => {
    ...
    expect(decision).toMatchObject({
      source: "independent",
      verdict: "passed",
      write: { operation: "wf.verify-task", input: { source: "independent", verdict: "passed", by: "review-agent" } },
    });
  ...
  it("拒绝执行者自裁和缺失任一审查轴", () => {
    expect(() =>
      adjudicateWorkflowTask({ taskId: "x", reviewer: "same", executor: "same", specificationEvidence: ["ok"], conventionEvidence: ["ok"] }),
    ).toThrow("ADJUDICATION_NOT_INDEPENDENT");
    expect(() =>
      adjudicateWorkflowTask({ taskId: "x", reviewer: "reviewer", specificationEvidence: [], conventionEvidence: ["ok"] }),
    ).toThrow("ADJUDICATION_EVIDENCE_REQUIRED");
  });
});
