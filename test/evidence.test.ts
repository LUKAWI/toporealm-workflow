...
// ---------- evidence.test.ts 原样移植（纯领域函数，语义零损失） ----------

describe("workflow 证据核心", () => {
  it("按 checkpoint 小状态聚合完成、失败和人工等待", () => {
    const checkpoints: WorkflowCheckpoint[] = [
      { id: "plan", taskId: "task-1", title: "计划", status: "passed", verifier: "self" },
      { id: "review", taskId: "task-1", title: "复核", status: "pending", verifier: "human" },
      { id: "docs", taskId: "task-1", title: "文档", status: "skipped", verifier: "independent" },
    ];
  ...
  it("checkpoint 与 report 按各自 id 独立寻址，并保留任务归属", () => {
    const checkpoint: WorkflowCheckpoint = { id: "cp-1", taskId: "task-1", title: "实现", status: "pending", verifier: "self" };
    const report: WorkflowExecutionReport = {
      id: "report-1",
      taskId: "task-1",
      summary: "已完成实现",
      artifacts: ["src/evidence.ts"],
      blockers: [],
      notes: "可交接",
  ...
  it("任务聚合读模型同步 checkpoint/report 计数与验证来源", () => {
    const task: WorkflowTask = {
      ...
      verification: { source: "self", verdict: "passed", by: "worker", at: "2026-09-11T00:00:00.000Z" },
    };
    const checkpoint: WorkflowCheckpoint = { id: "cp-1", taskId: task.id, title: "实现", status: "passed", verifier: "self" };
    const report: WorkflowExecutionReport = {
      id: "report-1",
      taskId: task.id,
      summary: "已完成",
  ...
  it("self 证据可完成任务，建议 independent 复核不阻塞完成", () => {
    ...
    const checkpoint: WorkflowCheckpoint = { id: "cp-self", taskId: task.id, title: "自验", status: "passed", verifier: "self", by: "worker" };
    const report: WorkflowExecutionReport = {
      id: "report-self",
      taskId: task.id,
      summary: "自验完成",
    ...
    const independent: WorkflowVerification = {
      source: "independent",
      verdict: "pending",
      by: "adjudicator",
    ...
    expect(suggestIndependentReview(task)).toMatchObject({ suggested: true, source: "independent", blocking: false });
  ...
  it("记录 verification 时保留 self 与 independent 来源", () => {
    ...
    const selfVerified = recordVerification(task, { source: "self", verdict: "passed", actor: "agent", by: "worker", at: "2026-09-11T00:00:00.000Z" });
    const independentVerified = recordVerification(task, { source: "independent", verdict: "passed", actor: "agent", by: "adjudicator", at: "2026-09-11T00:00:01.000Z" });
    const humanVerified = recordVerification(task, { source: "human", verdict: "passed", actor: "user", by: "alice", at: "2026-09-11T00:00:02.000Z" });
  ...
  it("human checkpoint 未确认时阻止完成，agent 不能代签，用户确认后才放行", () => {
    ...
    const humanCheckpoint: WorkflowCheckpoint = { id: "cp-human", taskId: task.id, title: "用户确认", status: "pending", verifier: "human" };
    const report: WorkflowExecutionReport = {
      id: "report-human",
      taskId: task.id,
      summary: "等待人工确认",
    ...
    expect(() => updateCheckpoint(humanCheckpoint, "passed", { actor: "agent", by: "worker" })).toThrow("HUMAN_CONFIRMATION_REQUIRED");
    ...
    expect(() =>
      recordVerification(task, { source: "human", verdict: "passed", actor: "agent", by: "worker" }),
