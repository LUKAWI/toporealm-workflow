// ---------- workflow 证据核心：checkpoint 聚合、完成门禁、verification（0.x evidence/ 原样移植） ----------
//
// 纯领域函数：只消费公开领域对象，不读不写图（图写入属于命令 + api.commit）。
// 独立复核（independent）只是可见建议，不改变完成门禁；human 证据必须来自用户真实确认。
...
/** checkpoint 相同状态重复上报幂等；其余状态必须遵守小状态机（再导出供语义测试与调用方使用）。 */
export { canTransitionCheckpoint };
...
export function aggregateCheckpoints(checkpoints: readonly WorkflowCheckpoint[]): CheckpointAggregate {
  const counts: Record<CheckpointStatus, number> = { pending: 0, running: 0, passed: 0, failed: 0, skipped: 0 };
  const byVerifier: Record<VerificationSource, number> = { self: 0, independent: 0, human: 0 };
  ...
  else if (checkpoints.length > 0 && checkpoints.every((c) => isTerminalCheckpoint(c.status))) status = "passed";
  else if (counts.running > 0) status = "running";

  const requiresHuman = checkpoints.some(
    (checkpoint) => checkpoint.verifier === "human" && !isTerminalCheckpoint(checkpoint.status),
  );
...
export function updateCheckpoint(
  ...
  if (
    checkpoint.verifier === "human" &&
    isTerminalCheckpoint(status as CheckpointStatus) &&
    actor !== "user"
  ) {
...
export function isUserIdentity(by: string | undefined): boolean {
  if (!by) return false;
  const normalized = by.trim().toLowerCase();
  return normalized === "user" || normalized === "human" || normalized.startsWith("user:") || normalized.startsWith("human:");
}
 ...
 * （1.0：同一逻辑被 before-commit 钩子在 after 快照上复用——门禁对一切前向来源生效。）
 */
export function assessTaskCompletion(
  ...
  if (
    humanCheckpoints.some(
      (checkpoint) => !isTerminalCheckpoint(checkpoint.status) || !isUserIdentity(checkpoint.by),
    )
  ) {
    blockers.push("human checkpoint 尚未由用户确认");
  }

  const evidence = [...(task.verification ? [task.verification] : []), ...verifications];
